"""Phase 4 / Tier 2 — possessions per-person accessor (Avalonia vs Access)."""
...
def test_possessions_pair_smoke_end_to_end(tmp_path: Path) -> None:
    ...
    from cbdb_parity.access_possessions import possessions_query_access
    from cbdb_parity.avalonia_possessions import possessions_field_names, possessions_query
    from cbdb_parity.diff_report import diff_rows, write_report

    # Su Shi (1762) has no possessions records; person 562505 has 3.
    person_id = 562505

    avalonia_data = cfg.avalonia_repo / "Cbdb.App.Data"
    avalonia_rows = possessions_query(
        sqlite_path,
        person_id,
        avalonia_data_dir=avalonia_data,
    )
    access_rows = possessions_query_access(mdb_path, person_id)

    diff = diff_rows(
        avalonia_rows,
        access_rows,
        key_fields=("sequence", "record_id"),
        compare_fields=possessions_field_names(),
    )

    reports_dir = Path.cwd() / "reports"
    write_report(
        reports_dir,
        query_id="possessions_basic_person",
    ...
    assert diff.stats.matches, (
        f"possessions-pair diff for person_id={person_id} has mismatches: "
        ...
        f"See reports/possessions_basic_person/ for detail."
    )
