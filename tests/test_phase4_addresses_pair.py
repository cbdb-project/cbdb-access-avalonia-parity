"""Phase 4 / Tier 2 — addresses per-person accessor (Avalonia vs Access).

Mirror of `test_phase4_altnames_pair` / `test_phase4_entries_pair`.
Person 1762 (Su Shi) has multiple BIOG_ADDR_DATA rows (basic +
postings + natal) — small enough to inspect, exercises the natal
flag + nianhao + ganzhi + range JOINs.
"""
...
def test_addresses_pair_smoke_end_to_end(tmp_path: Path) -> None:
    ...
    from cbdb_parity.access_addresses import addresses_query_access
    from cbdb_parity.avalonia_addresses import addresses_field_names, addresses_query
    from cbdb_parity.diff_report import diff_rows, write_report

    person_id = 1762

    avalonia_data = cfg.avalonia_repo / "Cbdb.App.Data"
    avalonia_rows = addresses_query(
        sqlite_path,
        person_id,
        avalonia_data_dir=avalonia_data,
    )
    access_rows = addresses_query_access(mdb_path, person_id)

    # Diff key uses raw IDs (`addr_type_code`, `addr_id`) plus
    # `sequence` — BIOG_ADDR_DATA's natural unique-per-person key
    # (same as the ORDER BY in the Avalonia SQL).
    diff = diff_rows(
        avalonia_rows,
        access_rows,
        key_fields=("sequence", "addr_type_code", "addr_id"),
        compare_fields=addresses_field_names(),
    )

    reports_dir = Path.cwd() / "reports"
    write_report(
        reports_dir,
        query_id="addresses_basic_person",
    ...
    assert diff.stats.matches, (
        f"addresses-pair diff for person_id={person_id} has mismatches: "
        ...
        f"See reports/addresses_basic_person/ for detail."
    )
