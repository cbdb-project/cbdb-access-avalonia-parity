"""Phase 4 / Tier 2 — writings per-person accessor (Avalonia vs Access)."""
...
def test_writings_pair_smoke_end_to_end(tmp_path: Path) -> None:
    ...
    from cbdb_parity.access_writings import writings_query_access
    from cbdb_parity.avalonia_writings import writings_field_names, writings_query
    from cbdb_parity.diff_report import diff_rows, write_report

    person_id = 1762

    avalonia_data = cfg.avalonia_repo / "Cbdb.App.Data"
    avalonia_rows = writings_query(
        sqlite_path,
        person_id,
        avalonia_data_dir=avalonia_data,
    )
    access_rows = writings_query_access(mdb_path, person_id)

    diff = diff_rows(
        avalonia_rows,
        access_rows,
        key_fields=("year", "text_id", "role_id"),
        compare_fields=writings_field_names(),
    )

    reports_dir = Path.cwd() / "reports"
    write_report(
        reports_dir,
        query_id="writings_basic_person",
    ...
    assert diff.stats.matches, (
        f"writings-pair diff for person_id={person_id} has mismatches: "
        ...
        f"See reports/writings_basic_person/ for detail."
    )
