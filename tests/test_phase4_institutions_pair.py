"""Phase 4 / Tier 2 — institutions per-person accessor (Avalonia vs Access)."""
...
def test_institutions_pair_smoke_end_to_end(tmp_path: Path) -> None:
    ...
    from cbdb_parity.access_institutions import institutions_query_access
    from cbdb_parity.avalonia_institutions import institutions_field_names, institutions_query
    from cbdb_parity.diff_report import diff_rows, write_report

    # Su Shi (1762) has no institution affiliations; person 551355
    # has the most (4 rows).
    person_id = 551355

    avalonia_data = cfg.avalonia_repo / "Cbdb.App.Data"
    avalonia_rows = institutions_query(
        sqlite_path,
        person_id,
        avalonia_data_dir=avalonia_data,
    )
    access_rows = institutions_query_access(mdb_path, person_id)

    diff = diff_rows(
        avalonia_rows,
        access_rows,
        key_fields=("begin_year", "inst_name_code", "inst_code"),
        compare_fields=institutions_field_names(),
    )

    reports_dir = Path.cwd() / "reports"
    write_report(
        reports_dir,
        query_id="institutions_basic_person",
    ...
    assert diff.stats.matches, (
        f"institutions-pair diff for person_id={person_id} has mismatches: "
        ...
        f"See reports/institutions_basic_person/ for detail."
    )
