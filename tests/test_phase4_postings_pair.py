"""Phase 4 / Tier 2 — postings per-person accessor (Avalonia vs Access).

NB: this test diffs the RAW SQL output (50 columns), not the post-
processed `Posting → Office → Address` nested structure. The C#
accumulator is deterministic Python-portable logic applied identically
to both sides, so raw-row equality implies post-processed equality.
"""
...
def test_postings_pair_smoke_end_to_end(tmp_path: Path) -> None:
    ...
    from cbdb_parity.access_postings import postings_query_access
    from cbdb_parity.avalonia_postings import postings_field_names, postings_query
    ...
    diff = diff_rows(
        avalonia_rows,
        access_rows,
        key_fields=("posting_id", "sequence", "office_id", "addr_id"),
        compare_fields=postings_field_names(),
    )

    reports_dir = Path.cwd() / "reports"
    write_report(
        reports_dir,
        query_id="postings_basic_person",
    ...
    assert diff.stats.matches, (
        f"postings-pair diff for person_id={person_id} has mismatches: "
        ...
        f"See reports/postings_basic_person/ for detail."
    )
