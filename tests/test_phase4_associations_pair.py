"""Phase 4 / Tier 2 — associations per-person accessor (Avalonia vs Access)."""
...
def test_associations_pair_smoke_end_to_end(tmp_path: Path) -> None:
    ...
    from cbdb_parity.access_associations import associations_query_access
    from cbdb_parity.avalonia_associations import associations_field_names, associations_query
    from cbdb_parity.diff_report import diff_rows, write_report

    person_id = 1762

    avalonia_data = cfg.avalonia_repo / "Cbdb.App.Data"
    avalonia_rows = associations_query(
        sqlite_path,
        person_id,
        avalonia_data_dir=avalonia_data,
    )
    access_rows = associations_query_access(mdb_path, person_id)

    # ASSOC_DATA has no PRAGMA PK and multiple texts may document the
    # same (year, sequence, assoc_code, associate_id) association.
    # `text_title` is the discriminator that makes the key unique on
    # the Su Shi sample (0 duplicate groups). Including it in key
    # _and_ compare is harmless — matched-key rows necessarily agree
    # on text_title, and a real text_title divergence would surface as
    # only-in-one-side rather than as a value mismatch.
    diff = diff_rows(
        avalonia_rows,
        access_rows,
        key_fields=("year", "sequence", "assoc_code", "associate_person_id", "text_title"),
        compare_fields=associations_field_names(),
    )

    reports_dir = Path.cwd() / "reports"
    write_report(
        reports_dir,
        query_id="associations_basic_person",
    ...
    assert diff.stats.matches, (
        f"associations-pair diff for person_id={person_id} has mismatches: "
        ...
        f"See reports/associations_basic_person/ for detail."
    )
