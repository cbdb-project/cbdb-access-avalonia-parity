"""Phase 4 / Tier 2 — events per-person accessor (Avalonia vs Access)."""
...
def test_events_pair_smoke_end_to_end(tmp_path: Path) -> None:
    ...
    from cbdb_parity.access_events import events_query_access
    from cbdb_parity.avalonia_events import events_field_names, events_query
    from cbdb_parity.diff_report import diff_rows, write_report

    person_id = 1762

    avalonia_data = cfg.avalonia_repo / "Cbdb.App.Data"
    avalonia_rows = events_query(
        sqlite_path,
        person_id,
        avalonia_data_dir=avalonia_data,
    )
    access_rows = events_query_access(mdb_path, person_id)

    diff = diff_rows(
        avalonia_rows,
        access_rows,
        key_fields=("year", "sequence", "event_code"),
        compare_fields=events_field_names(),
    )

    reports_dir = Path.cwd() / "reports"
    write_report(
        reports_dir,
        query_id="events_basic_person",
    ...
    assert diff.stats.matches, (
        f"events-pair diff for person_id={person_id} has mismatches: "
        ...
        f"See reports/events_basic_person/ for detail."
    )
