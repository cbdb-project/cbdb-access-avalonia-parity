"""Phase 4 / Tier 2 — detail per-person accessor (Avalonia vs Access).

Diffs the 26-column SELECT + 12 COUNT subqueries of GetDetailAsync.
The Avalonia `Fields` list (FK-enriched BIOG_MAIN dump) is not
replicated here.
"""
...
def test_detail_pair_smoke_end_to_end(tmp_path: Path) -> None:
    ...
    from cbdb_parity.access_detail import detail_query_access
    from cbdb_parity.avalonia_detail import detail_field_names, detail_query
    from cbdb_parity.diff_report import diff_rows, write_report

    person_id = 1762

    avalonia_data = cfg.avalonia_repo / "Cbdb.App.Data"
    avalonia_rows = detail_query(
        sqlite_path,
        person_id,
        avalonia_data_dir=avalonia_data,
    )
    access_rows = detail_query_access(mdb_path, person_id)

    diff = diff_rows(
        avalonia_rows,
        access_rows,
        key_fields=("person_id",),
        compare_fields=detail_field_names(),
    )

    reports_dir = Path.cwd() / "reports"
    write_report(
        reports_dir,
        query_id="detail_basic_person",
    ...
    assert diff.stats.matches, (
        f"detail-pair diff for person_id={person_id} has mismatches: "
        ...
        f"See reports/detail_basic_person/ for detail."
    )
