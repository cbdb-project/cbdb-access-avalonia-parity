...
|---|---|---|---|---|
| 1 | **Biographic basic** | _(no Access test-driven form; closest analogue is Form_LookAtPeople which has no automated coverage)_ | `IPersonBrowserService.SearchAsync(keyword, limit, offset)` | 🆕 partially-paired (Phase 3 will issue equivalent SQL on the mdb side directly) |
...
| 6 | **Associations** | `assoc_basic_search` · Form_LookAtAssociations · CmdQuery — *code/dynasty-filtered people list* | `IPersonBrowserService.GetAssociationsAsync(personId)` — *per-person associations* | ⚠️ shape-mismatched (Phase 3 must define the SAME question on both sides — e.g. "associations of person X" issued directly against each backend, NOT the form's default behaviour) |
| 7 | **Kinship** | `kinship_recursive_traversal` · Form_LookAtKinship · CmdRun — *recursive expansion from seed* | `IPersonBrowserService.GetKinshipsAsync(personId, expandNetwork)` — only **1-hop** even with `expandNetwork=true` per avalonia_queries.yaml gaps list | ⚠️ shape-mismatched (recursive vs 1-hop; Phase 3 must either restrict Access to 1-hop or add a recursive method on the Avalonia side) |
...
- **Directly-comparable pairs (Phase 3 ready)**: Tier 1 = 3 ✅ (entry/office/status query — strict same-shape) + Tier 2 = 13 ✅ direct-SQL per-person accessors = **16 query pairs** the harness can drive without further shape negotiation
- **Partial / shape-mismatched** (Phase 3 must narrow before driving): 4 — BIOG basic (no Access test-driven form; needs custom SQL fixture), associations (code-filter vs per-person), kinship (recursive vs 1-hop), GroupData (demographics vs grouped relations)
- **Misaligned (different shapes)**: 1 — GroupData demographics vs GroupPeople aggregation
- **Access-only (no Avalonia analogue)**: 4 query categories (Texts, Networks, AssociationPairs, Place) — major Phase 4 expansion targets
